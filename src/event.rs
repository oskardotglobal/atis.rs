...
use serenity::all::{
    ActionExecution, CreateAttachment, CreateMessage, CreateWebhook, ExecuteWebhook, FullEvent,
};
use serenity::builder::Builder;
use std::str::from_utf8;

const COMPLIMENTS: [&str; 200] = [
"Hey there sexy!",
...
pub(crate) async fn event_handler(
    ...
    match event {
        FullEvent::Message { new_message } => handle_message(new_message, data, ctx).await,
        FullEvent::AutoModActionExecution { execution } => {
            handle_automod_event(execution, ctx).await
        }
        _ => (),
    };
...
/// Handle messages
/// Currently only checks for custom commands
pub async fn handle_message(new_message: &Message, data: &Data, ctx: &Context) {
    if new_message.guild(ctx.cache.as_ref()).is_none() {
        return;
    }
    ...
    if content.is_some() {
        handle_custom_commands(
            content.unwrap().to_ascii_lowercase(),
            new_message,
            data,
            ctx,
        )
...
pub async fn handle_automod_event(execution: &ActionExecution, ctx: &Context) {
    let channel_id = match execution.channel_id {
        Some(id) => id,
        None => return,
    };
    ...
    let author = match ctx
        .http
        .get_member(execution.guild_id, execution.user_id)
        .await
    {
        Ok(member) => member,
        Err(_) => return,
    ...
    let webhook = match CreateWebhook::new(author.display_name())
        .avatar(&CreateAttachment::url(&ctx, &author.face()).await.unwrap())
    ...
    let _ = match fastrand::choice(COMPLIMENTS) {
        None => return,
        Some(compliment) => {
            webhook
                .execute(&ctx, false, ExecuteWebhook::new().content(compliment))
                .await
        }
    };
...
pub async fn handle_custom_commands(
    ...
    match data.tree.get(command) {
        Ok(Some(response)) => {
            let content = from_utf8(&response).unwrap_or_else(|e| {
